...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while self.place is not beehive and index <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            inflict = amount + self.damage
            self.place.remove_insect(self)
            self.death_callback()

        for bee in list(bee_list):
            bee.reduce_armor(inflict)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        self.digesting += self.digest_time
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.queen_num == 1:
            ...
            while hasattr(loc, 'exit'):
                loc = loc.exit
                places.append(loc)
            for place in places:
                if place and place.ant:
                    if place.ant.is_container:
                        if place.ant.dd == False:
                            place.ant.damage *= 2
                            place.ant.dd = True
                        if place.ant.contained_ant and place.ant.contained_ant.dd == False:
                            place.ant.contained_ant.damage *=2
                            place.ant.contained_and.dd = True
                    if place.ant.dd == False:
                        place.ant.damage *= 2
                        place.ant.dd = True
        else:
...
class AntColony(object):
    ...
    def __init__(self, strategy, beehive, ant_types, create_places, dimensions, food=20):
